...
from sklearn.metrics import classification_report, accuracy_score,precision_recall_fscore_support, recall_score, f1_score
...
class SVM_argClassification():
    def __init__(self, path, datareader=None, vec=None, clf=None, build=True):
        ...
        self.clf = clf if clf is not None else make_pipeline(StandardScaler(with_mean=False),RandomForestClassifier())#, svm.SVC(gamma='auto'))
    ...
    def prep_data_arg(self):
        ...
        mapper = DataFrameMapper([('sent_vector', self.vectoriser_mod),
                                  ...
                                  (['pos_feature', 'puct_num','overall_index'], None)])
...
def run():
    ...
    # svm type classifier
    # print(x_test, y_test)
    # svm_type = SvmTypeClassifier(reader.load_from_directory(True))
    ...
    if ml_model is None:
        ...
        data = ml_model.data
    svm_relation = SvmRelationship(data, build=True)
